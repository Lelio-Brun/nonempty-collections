...
///
/// The [`neis`] macro is the simplest way to construct an `NEIndexSet`:
///
/// ```
/// use nonempty_collections::*;
///
/// let s = neis![1, 1, 2, 2, 3, 3, 4, 4];
...
/// use nonempty_collections::neis;
///
/// let s = neis!["Fëanor", "Fingolfin", "Finarfin"];
    ...
    S: BuildHasher,
{
    ...
    /// use nonempty_collections::neis;
    ///
    /// let s: IndexSet<_> = neis![1, 2, 3].into();
    ...
    T: Eq + Hash,
{
    ...
    /// ```
    /// use nonempty_collections::{NEIndexSet, Singleton, neis};
    ///
...
mod test {
    ...
    fn iter_debug_impl() {
        ...
        let actual = format!("{:?}", neis! {0}.nonempty_iter());
        assert_eq!(expected, actual);
    }
}
